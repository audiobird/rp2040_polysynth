...
void voice_allocator_note_on(uint8_t channel, uint8_t note, uint8_t velocity)
{
    if (valloc.params->method == VALLOC_BLOCK && valloc.active_voices == SYNTH_NUM_VOICES)
    return;

    uint8_t best_voice = 0;
...
void voice_allocator_note_off(uint8_t channel, uint8_t note)
{
    for (int x = 0; x < SYNTH_NUM_VOICES; x++)
    {
